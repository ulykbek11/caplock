...
export async function doctor(root: string): Promise<Check[]> {
  ...
  try { const pnpm = process.platform === "win32" ? await run("cmd.exe", ["/d", "/s", "/c", "pnpm.cmd --version"]) : await run("pnpm", ["--version"]); checks.push({ name: "pnpm", ok: pnpm.code === 0, detail: pnpm.code === 0 ? pnpm.stdout.trim() : "pnpm not found (enable Corepack or install pnpm)" }); } catch { checks.push({ name: "pnpm", ok: false, detail: "pnpm not found (enable Corepack or install pnpm)" }); }
